  ...
  table: {
    columns: [
      ...
      { label: "进度", field: "progress" }
        ...
        customContent: `完整表格配置可以参考 <a href="/api/options#table">Table API</a>`
      },
      {
        id: "time-axis",
        title: "时间轴",
  ...
  links: {
    ...
    create: {
      ...
      mode: 'hover'               // 启用创建，则可以配置连接点的展示方式。 hover | always
          ...
          {
            customContent: `完整关连线配置可以参考 <a href="/api/options#links">Links API</a>`
          },
          {
            title: "依赖关系的使用",
...
gantt.on("select:link", (add, cancel, all) => {
  if (add?.index === "555") {
    links.splice(links.findIndex(l => l.index === add.index), 1);  // 模拟选中删除
          ...
          {
            title: "依赖关系的性能问题",
            customContent: "<code>XGantt</code> 内部使用了 <code>Konva</code> 库进行渲染，当绘制大量依赖关系时，会出现页面卡顿现象，这是因为 <code>Konva</code> 在处理虚线时，会计算大量位置关系，尤其是像我们的依赖关系这样的多段折线。<div style=\"margin-top: 12px;\">● 如果您的项目需要大量连线，我们十分建议您使用实线，这样会大幅度提升性能。</div><div>● 如果您确实需要使用虚线，可以尝试设置为 [5, 5] 这样的等宽值，它会相应的减少一些计算量。</div><div>● 当然，您还可以尝试使用更细的线条，这样也会对性能优化有很大帮助。</div>"
          },
        ]
      },
      {
        ...
        subsections: [
          {
            list: [
              ...
              "**依赖关系**：依赖关系是一个十分消耗性能的渲染功能，尽可能避免使用虚线绘制，具体可以查看 <a href=\"/tutorials#依赖关系的性能问题\">依赖关系的性能问题</a>"
            ]
          }
        ]
      }
