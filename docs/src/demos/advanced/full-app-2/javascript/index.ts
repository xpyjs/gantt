...
const gantt = new XGantt(ganttContainer, {
  ...
  baselines: {
    ...
    compare: {
      enabled: true,
      ahead: { backgroundColor: '#52c41a' },
      delayed: { backgroundColor: '#ff4d4f' },
      indicator: {
        show: true,
        ahead: { opacity: 0.5, text: (diff: number, row: any) => `领先 ${diff.toFixed(1)} 天` },
        delayed: { text: (diff: number, row: any) => `超期 ${-diff.toFixed(1)} 天` }
      }
    }
  },
  ...
  holiday: {
    ...
    holidays: [
      {
        date: '2025-09-03',
        text: { show: true, content: '抗日战争胜利纪念日', backgroundColor: '#f5222dcc' },
        pattern: 'custom',
        opacity: 0.2,
        patternOptions: { image: 'https://picsum.photos/id/383/50' }
      }
    ]
  }
});
...
gantt.on("update:link", (link) => {
  ...
  if (index > -1) { links.splice(index, 1, link as any); notify('info', '已更新依赖关系'); }
});
gantt.on("click:row", (_e, item) => { selected = item; updateFooterSelection(); updateActionBtnState(); });
gantt.on("select", (_data, _checked, all) => { checkedList = all; updateFooterChecked(); });

function findTaskById(id: string | number, list = data): any | null {
  for (const t of list) {
    if (t.id === id) return t;
    if (Array.isArray(t.subtask) && t.subtask.length) { const r = findTaskById(id, t.subtask); if (r) return r; }
  }
  return null;
}

interface TaskFormValues { name: string; start: string; end: string; progress: number; type: 'task' | 'milestone'; }
...
const taskDialog = (() => {
  ...
  panel.innerHTML = `
    ...
    <form id="task-form" autocomplete="off" novalidate>
      <div class="task-dialog-fields">
        <label class="field"><span class="field-label">名称</span><input name="name" type="text" maxlength="120" required /></label>
        <div class="field-row">
          <label class="field"><span class="field-label">开始时间</span><input name="start" type="date" required /></label>
          <label class="field"><span class="field-label">结束时间</span><input name="end" type="date" required /></label>
        </div>
        <label class="field"><span class="field-label">进度 (0-100)</span><input name="progress" type="number" min="0" max="100" value="0" /></label>
        <label class="field"><span class="field-label">类型</span><select name="type"><option value="task">普通任务</option><option value="milestone">里程碑</option></select></label>
      </div>
  ...
  function syncMilestone() {
    if (typeSelect.value === 'milestone') { endInput.disabled = true; endInput.value = startInput.value || dayjs().format('YYYY-MM-DD'); }
    else endInput.disabled = false;
  }
  typeSelect.addEventListener('change', syncMilestone);
  startInput.addEventListener('change', () => { if (typeSelect.value === 'milestone') endInput.value = startInput.value; });
  function baseInit() { const today = dayjs().format('YYYY-MM-DD'); nameInput.value = ''; startInput.value = today; endInput.value = today; progressInput.value = '0'; typeSelect.value = 'task'; endInput.disabled = false; }
  ...
  function open() { overlay.style.display = 'flex'; document.addEventListener('keydown', escHandler); nameInput.focus(); }
  function hide() { overlay.style.display = 'none'; document.removeEventListener('keydown', escHandler); }
  btnCancel.addEventListener('click', hide);
  btnClose.addEventListener('click', hide);

  form.addEventListener('submit', (e) => {
    e.preventDefault();
    const values: TaskFormValues = { name: sanitize(nameInput.value.trim()), start: startInput.value, end: endInput.value, progress: Number(progressInput.value || 0), type: (typeSelect.value as any) === 'milestone' ? 'milestone' : 'task' };
    ...
    else { if (!values.end) return notify('error', '结束时间不能为空'); if (dayjs(values.end).isBefore(dayjs(values.start))) return notify('error', '结束时间不能早于开始时间'); }
    if (Number.isNaN(values.progress) || values.progress < 0 || values.progress > 100) return notify('error', '进度需为 0-100 的数字');

    if (mode === 'create') {
      const id = generateId();
      data.push({ id, name: values.name, start: dayjs(values.start).format(DATE_FMT), end: dayjs(values.end).format(DATE_FMT), progress: values.progress, subtask: [], type: values.type });
      gantt.update({ data }); updateFooterStatusInfo(); notify('success', '已创建任务'); hide();
      ...
      gantt.update({ data }); notify('success', '已保存任务'); hide();
    } else if (mode === 'createChild' && parentTask) {
      const id = generateId();
      const child = { id, name: values.name, start: dayjs(values.start).format(DATE_FMT), end: dayjs(values.end).format(DATE_FMT), progress: values.progress, subtask: [], type: values.type };
      if (!Array.isArray(parentTask.subtask)) parentTask.subtask = [];
      parentTask.subtask.push(child);
      gantt.update({ data }); updateFooterStatusInfo(); notify('success', '已创建子任务'); hide();
    }
  });
...
interface LinkFormValues { type: 'FS' | 'FF' | 'SS' | 'SF'; color: string; lag: number; }
const linkDialog = (() => {
  ...
  panel.innerHTML = `
    ...
    <form id="link-form" autocomplete="off" novalidate>
      <div class="task-dialog-fields">
        <div class="field-row"><label class="field"><span class="field-label">From</span><input name="from" type="text" disabled /></label><label class="field"><span class="field-label">To</span><input name="to" type="text" disabled /></label></div>
        <label class="field"><span class="field-label">类型 (依赖关系)</span><div class="radio-group" data-role="type-group">${ALLOW_TYPES.map(t => `<label class="radio-chip"><input type="radio" name="linkType" value="${t}" /><span>${t}</span></label>`).join('')}</div></label>
        <label class="field"><span class="field-label">颜色</span><div class="color-swatch-group" data-role="color-group">${COLORS.map(c => `<label class="color-swatch" style="--sw:${c}"><input type="radio" name="linkColor" value="${c}"><span></span></label>`).join('')}</div></label>
        <label class="field"><span class="field-label">延迟 (天, 可负数)</span><input name="lag" type="number" step="1" min="-365" max="365" value="0" /></label>
      </div>
      <div class="task-dialog-footer"><button type="button" class="btn-secondary" data-action="cancel">取消</button><button type="submit" class="btn-primary" data-role="submit">保存</button></div>
    </form>
    <button type="button" class="task-dialog-close" aria-label="关闭" data-action="close"></button>
  `;
  overlay.appendChild(panel);
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (!typeRadio || !ALLOW_TYPES.includes(typeRadio.value as any)) return notify('error', '请选择合法依赖类型');
    if (!colorRadio || !COLORS.includes(colorRadio.value)) return notify('error', '请选择合法颜色');
    if (!Number.isFinite(lagNum) || lagNum < -365 || lagNum > 365) return notify('error', '延迟需在 -365 ~ 365 之间');
...
function updateActionBtnState() { if (editBtn) editBtn.disabled = !selected; if (createChildBtn) createChildBtn.disabled = !selected; }

if (btnGroup) {
  const btnToday = btnGroup.querySelector('#btn-today') as HTMLButtonElement;
  btnToday.addEventListener('click', () => { gantt.jumpTo(); notify('info', '已跳转到今天'); });
  ...
  createChildBtn.addEventListener('click', () => { if (!selected) return; const real = findTaskById(selected.id) || selected; if (real.type === 'milestone') return notify('warning', '里程碑下不能创建子任务'); taskDialog.showCreateChild(real); });
  editBtn = btnGroup.querySelector('#btn-open-edit-dialog') as HTMLButtonElement;
  editBtn.addEventListener('click', () => { if (!selected) return; const realTask = findTaskById(selected.id) || selected; taskDialog.showEdit(realTask); });
  const colorSelect = btnGroup.querySelector('#select-primary-color') as HTMLSelectElement;
  colorSelect.addEventListener('change', () => { gantt.update({ primaryColor: colorSelect.value }); notify('info', '主题色已更新'); });
  const localeSelect = btnGroup.querySelector('#select-locale') as HTMLSelectElement;
  localeSelect.addEventListener('change', () => { gantt.update({ locale: localeSelect.value }); notify('info', '语言已更新'); });
  const unitSelect = btnGroup.querySelector('#select-unit') as HTMLSelectElement;
  unitSelect.addEventListener('change', () => { gantt.update({ unit: unitSelect.value as any }); notify('info', '视图单位已更新'); });
  const holidayChk = btnGroup.querySelector('#chk-show-holiday') as HTMLInputElement;
  holidayChk.addEventListener('change', () => { gantt.update({ holiday: { show: holidayChk.checked } }); notify('info', '假期显示已更新'); });
  const weekendChk = btnGroup.querySelector('#chk-show-weekend') as HTMLInputElement;
  weekendChk.addEventListener('change', () => { gantt.update({ weekend: { show: weekendChk.checked } }); notify('info', '周末显示已更新'); });
  ...
  baselineBtn.addEventListener('click', () => { const open = baselinePanel.hasAttribute('hidden'); if (open) { baselinePanel.removeAttribute('hidden'); baselinePanel.style.display = 'block'; } else { baselinePanel.setAttribute('hidden', ''); baselinePanel.style.display = 'none'; } baselineBtn.setAttribute('aria-expanded', String(open)); });
  document.addEventListener('mousedown', e => { if (!baselinePanel.contains(e.target as any) && e.target !== baselineBtn) { baselinePanel.setAttribute('hidden', ''); baselinePanel.style.display = 'none'; baselineBtn.setAttribute('aria-expanded', 'false'); } });
  const baselineShowChk = btnGroup.querySelector('#chk-show-baseline') as HTMLInputElement;
  baselineShowChk.addEventListener('change', () => { gantt.update({ baselines: { show: baselineShowChk.checked } }); notify('info', baselineShowChk.checked ? '基线显示已开启' : '基线显示已关闭'); });
  const baselineCompareChk = btnGroup.querySelector('#chk-show-compare') as HTMLInputElement;
  baselineCompareChk.addEventListener('change', () => { gantt.update({ baselines: { compare: { enabled: baselineCompareChk.checked } } }); notify('info', baselineCompareChk.checked ? '基线对比已开启' : '基线对比已关闭'); });
  const baselineIndicatorChk = btnGroup.querySelector('#chk-show-indicator') as HTMLInputElement;
  baselineIndicatorChk.addEventListener('change', () => { gantt.update({ baselines: { compare: { indicator: { show: baselineIndicatorChk.checked } } } }); notify('info', baselineIndicatorChk.checked ? '基线对比指示器已开启' : '基线对比指示器已关闭'); });
}
...
function removeTaskById(id: string | number, list = data): boolean {
  ...
  for (const t of list) { if (Array.isArray(t.subtask) && t.subtask.length) { if (removeTaskById(id, t.subtask)) return true; } }
  return false;
}

interface ContextMenuItem { label?: string; onClick?: () => void; disabled?: boolean; danger?: boolean; divider?: boolean; }
...
function outsideHandler(e: MouseEvent) { if (!activeContextMenu) return; if (!activeContextMenu.contains(e.target as Node)) closeContextMenu(); }
function createContextMenu(items: ContextMenuItem[], x: number, y: number) {
  ...
  menu.tabIndex = -1;
  items.forEach(item => {
    if (item.divider) { const hr = document.createElement('div'); hr.className = 'context-menu-divider'; menu.appendChild(hr); return; }
    ...
    if (!item.disabled && item.onClick) btn.addEventListener('click', () => { try { item.onClick!(); } finally { closeContextMenu(); } });
  ...
  requestAnimationFrame(() => {
    ...
    menu.style.left = left + 'px'; menu.style.top = top + 'px'; menu.style.opacity = '1'; menu.focus();
  });
...
function buildRowMenu(row: any, e: MouseEvent) {
  ...
  const items: ContextMenuItem[] = [
    ...
    { label: '创建子任务', disabled: isMilestone, onClick: () => { if (isMilestone) return notify('warning', '里程碑下不能创建子任务'); taskDialog.showCreateChild(real); } },
    { label: isMilestone ? '转为普通任务' : (canConvertToMilestone ? '设为里程碑' : '设为里程碑 (需100%)'), disabled: !isMilestone && !canConvertToMilestone, onClick: () => { if (isMilestone) { real.type = 'task'; notify('success', '已还原为普通任务'); } else { if (!canConvertToMilestone) return notify('warning', '进度未完成，不能设为里程碑'); real.type = 'milestone'; notify('success', '已设为里程碑'); } gantt.update({ data }); } },
    { divider: true },
    { label: '删除任务', danger: true, onClick: () => { if (removeTaskById(real.id)) { if (selected && selected.id === real.id) selected = null; gantt.update({ data }); updateFooterStatusInfo(); updateFooterSelection(); updateActionBtnState(); notify('success', '任务已删除'); } else notify('error', '删除失败：未找到任务'); } }
  ];
...
function buildSliderMenu(row: any, e: MouseEvent) {
  ...
  const isMilestone = real.type === 'milestone';
  const canConvertToMilestone = !isMilestone && Number(real.progress) === 100;
  const quickSet = (val: number) => { if (real.progress === val) return notify('info', `进度已是 ${val}%`); real.progress = val; gantt.update({ data }); notify('success', `进度已设为 ${val}%`); };
  const items: ContextMenuItem[] = [
    ...
    { label: isMilestone ? '转为普通任务' : (canConvertToMilestone ? '设为里程碑' : '设为里程碑 (需100%)'), disabled: !isMilestone && !canConvertToMilestone, onClick: () => { if (isMilestone) { real.type = 'task'; notify('success', '已还原为普通任务'); } else { if (!canConvertToMilestone) return notify('warning', '进度未完成，不能设为里程碑'); real.type = 'milestone'; notify('success', '已设为里程碑'); } gantt.update({ data }); } },
    { divider: true },
    { label: '查看当前所有关联', onClick: () => { const dataChain = gantt.getDataChain(real.id); console.log('当前任务数据链：', dataChain); } },
    { divider: true },
    { label: '删除任务', danger: true, onClick: () => { if (removeTaskById(real.id)) { if (selected && selected.id === real.id) selected = null; gantt.update({ data }); updateFooterStatusInfo(); updateFooterSelection(); updateActionBtnState(); notify('success', '任务已删除'); } else notify('error', '删除失败：未找到任务'); } }
  ];
  createContextMenu(items, e.clientX, e.clientY);
...
function buildLinkMenu(link: any, e: MouseEvent) {
  const items: ContextMenuItem[] = [
    ...
    { label: '删除依赖', danger: true, onClick: () => { const idx = links.findIndex(l => l.id === link.id); if (idx > -1) { links.splice(idx, 1); gantt.update({ links: { data: links } as any }); notify('success', '依赖已删除'); } else notify('error', '未找到该依赖'); } }
  ];
  createContextMenu(items, e.clientX, e.clientY);
}

gantt.on('contextmenu:row', (e, row) => { e.preventDefault(); buildRowMenu(row, e); });
gantt.on('contextmenu:slider', (e, row) => { e.preventDefault(); buildSliderMenu(row, e); });
gantt.on('contextmenu:link', (e, link) => { e.preventDefault(); buildLinkMenu(link, e); });
