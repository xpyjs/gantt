import{d as G,i as I,o as i,c as r,t as m,f,F as k,r as w,a as d,e as x,j as T,k as X,l as A,u as D,m as R,n as E,p as U,q as M,_ as O}from"./main-D1UXgUrn.js";const b=70,_="  ",v=(t,p,l)=>{if(t===null)return"null";if(t===void 0)return"undefined";if(typeof t=="string")return`'${t}'`;if(["number","boolean","bigint"].includes(typeof t))return String(t);if(typeof t=="symbol")return t.toString();if(typeof t=="function"){const u=t.toString();return u.includes("[native code]")?`function ${t.name}() { [native code] }`:u}if(typeof t=="object"){if(l.has(t))return"'[Circular]'";l.add(t);const u=_.repeat(p),g=_.repeat(p+1);if(t instanceof Error)return l.delete(t),`new Error('${t.message}')`;if(t instanceof Date)return l.delete(t),`new Date('${t.toISOString()}')`;if(t instanceof RegExp)return l.delete(t),t.toString();let c;if(Array.isArray(t))if(t.length===0)c="[]";else{const s=t.map(a=>v(a,p+1,l)),n=`[${s.join(", ")}]`;c=n.length<=b?n:`[
${s.map(a=>`${g}${a}`).join(`,
`)}
${u}]`}else if(t instanceof Map)if(t.size===0)c="new Map()";else{const s=Array.from(t.entries()).map(([a,e])=>`[${v(a,p+1,l)}, ${v(e,p+1,l)}]`),n=`new Map([${s.join(", ")}])`;c=n.length<=b?n:`new Map([
${s.map(a=>`${g}${a}`).join(`,
`)}
${u}])`}else if(t instanceof Set)if(t.size===0)c="new Set()";else{const s=Array.from(t.values()).map(a=>v(a,p+1,l)),n=`new Set([${s.join(", ")}])`;c=n.length<=b?n:`new Set([
${s.map(a=>`${g}${a}`).join(`,
`)}
${u}])`}else{const s=Object.keys(t);if(s.length===0)c="{}";else{const n=s.map(e=>{const o=/^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(e)?e:`'${e}'`,h=v(t[e],p+1,l);return{keyStr:o,valStr:h}}),a=`{ ${n.map(e=>`${e.keyStr}: ${e.valStr}`).join(", ")} }`;a.length<=b?c=a:c=`{
${n.map(o=>`${g}${o.keyStr}: ${o.valStr}`).join(`,
`)}
${u}}`}}return l.delete(t),c}return String(t)},S=(t,p)=>{const l=v(t,0,new Set);return p?l.split(`
`).map((u,g)=>g===0?u:p+u).join(`
`):l},j=[{id:1,name:"项目规划",startTime:"2025-01-01",endTime:"2025-01-15",progress:100},{id:2,name:"开发阶段",startTime:"2025-01-16",endTime:"2025-02-28",progress:45}],B=`import { XGantt } from '@xpyjs/gantt-core'
...
<\/script>`,V=`import React from 'react'
...
export default App`,y={jsCode:B,vueCode:F,reactCode:V},L=`<!-- Vue 组件中使用 -->
<template>
  <div>
    <XGanttVue
...
export default GanttComponent;`,P=`// Angular 中使用。使用原生组件来实现
...
}`,language:"javascript"}]},{title:"自定义时间轴（scaleUnit）",description:"从 v0.1.0 开始，XGantt 支持通过 scaleUnit 配置自定义多层表头，可以灵活组合任意时间单位和步长，实现更精细的时间轴控制。"},{title:"与 unit 的关系",list:["**`unit`** 是简单模式，只需指定一个单位即可自动生成两层表头","**`scaleUnit`** 是高级模式，可自定义 N 层表头，每层独立配置单位、步长、格式化、行高等","两者**互斥**：配置了 `scaleUnit` 后，`unit` 将被忽略","已有的 `unit` 配置**无需修改**，`scaleUnit` 为新增能力，完全向后兼容","如果可以，建议优先使用 `scaleUnit` 进行更灵活的时间轴配置"]},{title:"基础用法",description:"scaleUnit 接收一个数组，从上（粗粒度）到下（细粒度）排列。每项可配置 unit、step、format 等属性。",code:[{framework:"javascript",code:`const options = {
  // 两层表头：上层按周，下层按天
  scaleUnit: [
    { unit: 'week', step: 1, format: 'YYYY 第ww周' },
    { unit: 'day', step: 1, format: 'DD' }
  ]
}`,language:"javascript"}]},{title:"三层表头 + 自定义步长与行高",description:"下面的示例展示了三层表头配置：月 → 日 → 8小时。底层每格 40px（8小时），月层行高固定 30px，其余两层自动平分。",code:[{framework:"javascript",code:`const options = {
  scaleUnit: [
    {
      unit: 'month',
      step: 1,
...
}`,language:"javascript"}]},{description:"渲染效果如下图，三层表头各层独立控制，拖拽扩展时表头保持稳定不偏移："},{customContent:'<img src="/tutorials-scale-unit.png" />'},{title:"配置要点",list:["**`cellWidth`** 仅允许在数组最后一项（底层 scale）设置，非底层的宽度由其包含的底层 cell 数量自动计算","**`height`** 可为每层单独指定行高（最小 20px），未指定的层自动平分 `header.height` 剩余空间","**`step`** 控制每格合并几个 unit，如 `step: 8, unit: 'hour'` 表示每格 8 小时","当所有层 height 之和超过 `header.height` 时，表头高度会自动扩展"]}],customContent:'完整时间轴的图表区域配置可以参考 <a href="/api/options#chart">Chart API</a>，自定义时间轴配置可参考 <a href="/api/options#scaleUnit">scaleUnit API</a> 和 <a href="/api/types#ScaleUnit">ScaleUnit 类型定义</a>'},{id:"task-bars",title:"任务条",description:"任务条是甘特图中 显示/操作 任务时间范围和进度的可视化元素。只要当前任务数据中包含起止时间，那么默认就会渲染对应的任务条。甘特图中的所有操作都是基于任务条的，所以它是甘特图的核心内容。",code:[{framework:"javascript",code:`const options = {
  bar: {
    height: 20,                     // 任务条高度
...
});`,language:"javascript"}],customContent:'完整的事件可以参考 <a href="/api/events">Event API</a>'}]},{id:"advanced-features",title:"高级功能",sections:[{id:"split-tasks",title:"单行多任务",description:"单行多任务（split）允许把一个任务的多个时间段（段，segment）渲染在同一行中，而不是展开为树形的多行。适用于施工排期、轮班计划等同一责任人在时间上断续推进的场景。",subsections:[{description:"开启 split 后，数据中 split 字段为真值的任务，其直接子级将作为段内联渲染在同一行：",list:["父任务无需提供起止时间，由所有段的最早开始与最晚结束自动派生（包络）","表格中 split 行不出现展开箭头，展开相关配置对其无效","未标记 split 的父任务保持普通树形展开语义，可与 split 任务共存","段与段、段与普通任务或里程碑之间均可建立依赖线，基线也可指向段"]},{description:"生效条件为：任务类型为普通 task、存在直接子级且无更深层级。字段名可通过 fields.split 自定义。",code:[{framework:"javascript",code:`const options = {
  split: {
...
}`,language:"javascript"}]},{description:"段的重叠策略（split.overlap）决定拖拽/缩放时段之间发生接触或交叠时的行为：free 不做约束；forbid 禁止交叠，交互被相邻段边界夹取；merge 在段接触时自动合并为一个段。"},{description:"merge 策略合并段时，指向被合并段的连线会重定向到保留段，自连、重复、成环的连线被移除。这些连线变更通过 update:link（携带 link 与 old 新旧数据）和 delete:link（携带被删连线）两个标准事件逐条抛出，可直接用于实现撤销。任务时间的变化则由 move 事件携带 { row, old }[] 数据提供。"},{customContent:'完整配置与事件说明可以参考 <a href="/api/options#split">Split API</a>，可运行的完整示例见 <a href="/demo/advanced/split-segment">单行多任务 Demo</a>'}]},{id:"custom-table",title:"定制表格",description:"表格支持丰富的自定义配置，包括多行表头、表格内容、以及自定义渲染等。",subsections:[{code:[{framework:"javascript",code:`const options = {
  table: {
    align: "left",           // 表格内容对齐方式
...
}`,language:"javascript"}]},{description:"渲染的内容如下图，可以看到表头、任务条、周末、假期等自定义后的显示效果："},{customContent:'<img src="/tutorials-custom-axis.png" />'}]},{id:"frameworks",title:"框架集成",description:"XGantt 可以很方便的集成到各种框架中。并且 XGantt 已经自带 vue 与 react 的框架",subsections:[{title:"Vue 集成",code:[{framework:"vue",language:"vue",code:C.vueCode},{framework:"react",language:"tsx",code:C.reactCode},{framework:"angular",language:"ts",code:C.angularCode}]}]},{id:"performance",title:"性能优化",description:"为了确保在大数据量下的流畅体验，XGantt 内置了多种性能优化策略。在多数情况，用户不用关心也不必担心性能问题，但是用户仍然可以通过编码使其更加优秀。",subsections:[{list:["**确保数据唯一键**：尽可能给每条数据添加唯一 ID，这会让 XGantt 处理数据更加高效","**只更新需要的内容**：尽可能不要每次全量更新 options，当数据没有变化时，不要传递它","**拖拽功能**：拖拽是一个耗时操作，非必要不开启，尽量启用按单位拖拽（byUnit）",'**依赖关系**：依赖关系是一个十分消耗性能的渲染功能，尽可能避免使用虚线绘制，具体可以查看 <a href="/tutorials#依赖关系的性能问题">依赖关系的性能问题</a>']}]}]}],H=()=>{const t=[];return $.forEach(p=>{t.push({id:p.id,title:p.title,sections:p.sections.map(l=>({id:l.id,title:l.title}))})}),t},z={class:"tutorial-content"},Y={key:0,class:"section-description"},W=["id"],q={key:1,class:"subsection-description"},K={key:2,class:"content-list"},Z=["innerHTML"],J={key:3,class:"config-table"},Q=["innerHTML"],tt=["innerHTML"],et=G({__name:"TutorialSection",props:{section:{}},setup(t){const p=I(),l=c=>{let s=c.replace(/`([^`]+)`/g,"<code>$1</code>");return s=s.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),u(s)},u=c=>{if(!c)return"";const s="/gantt/";let n=c.replace(/href="(\/[^"]*)"/g,(a,e)=>{const o=s.replace(/\/$/,""),h=e.startsWith("/")?e:`/${e}`;return`href="${o}${h}"`});return n=n.replace(/src="(\/[^"]*)"/g,(a,e)=>{const o=s.replace(/\/$/,""),h=e.startsWith("/")?e:`/${e}`;return`src="${o}${h}"`}),n},g=c=>{const s=c.target;if(s.tagName==="A"){const n=s.getAttribute("href");if(n){const a=new URL(n,window.location.origin);if(a.origin===window.location.origin){c.preventDefault();const e="/gantt/";let o=a.pathname;o.startsWith(e)&&(o=o.substring(e.length)),p.push(o+a.search+a.hash)}}}};return(c,s)=>(i(),r("div",z,[t.section.description?(i(),r("p",Y,m(t.section.description),1)):f("",!0),t.section.subsections?(i(!0),r(k,{key:1},w(t.section.subsections,(n,a)=>(i(),r("div",{key:a,class:"subsection"},[n.title?(i(),r("h3",{key:0,class:"subsection-title",id:n.title},m(n.title),9,W)):f("",!0),n.description?(i(),r("p",q,m(n.description),1)):f("",!0),n.list?(i(),r("ul",K,[(i(!0),r(k,null,w(n.list,(e,o)=>(i(),r("li",{key:o,innerHTML:l(e)},null,8,Z))),128))])):f("",!0),n.table?(i(),r("div",J,[d("table",null,[d("thead",null,[d("tr",null,[(i(!0),r(k,null,w(n.table.headers,e=>(i(),r("th",{key:e},m(e),1))),128))])]),d("tbody",null,[(i(!0),r(k,null,w(n.table.rows,e=>(i(),r("tr",{key:e.property},[d("td",null,[d("code",null,m(e.property),1)]),d("td",null,m(e.type),1),d("td",null,m(e.default),1),d("td",null,m(e.description),1)]))),128))])])])):f("",!0),n.code?(i(),x(T,{key:4,codeBlocks:n.code},null,8,["codeBlocks"])):f("",!0),n.customContent?(i(),r("div",{key:5,innerHTML:u(n.customContent),class:"custom-content",onClick:g},null,8,Q)):f("",!0)]))),128)):f("",!0),t.section.code?(i(),x(T,{key:2,codeBlocks:t.section.code},null,8,["codeBlocks"])):f("",!0),t.section.customContent?(i(),r("div",{key:3,innerHTML:u(t.section.customContent),class:"custom-content",onClick:g},null,8,tt)):f("",!0)]))}}),nt={class:"tutorials-page"},at={class:"tutorials-content"},ot={class:"tutorials-sidebar"},it={class:"tutorials-nav"},rt=["href","onClick"],st={class:"category-title"},lt=["id"],ct=G({__name:"Tutorials",setup(t){const p=H(),l=M(""),u=(n,a)=>{let e;return(...o)=>{clearTimeout(e),e=setTimeout(()=>n.apply(null,o),a)}},g=()=>{const n=[];return $.forEach(a=>{a.sections.forEach(e=>{n.push(e.id)})}),n},c=u(()=>{const n=g(),a=150;for(let e=n.length-1;e>=0;e--){const o=document.getElementById(n[e]);if(o&&o.getBoundingClientRect().top<=a){l.value=n[e];break}}},100),s=n=>{const a=document.getElementById(n);if(a){const h=a.offsetTop-80;window.scrollTo({top:h,behavior:"smooth"}),l.value=n}};return X(()=>{window.addEventListener("scroll",c,{passive:!0}),c()}),A(()=>{window.removeEventListener("scroll",c)}),(n,a)=>(i(),r("div",nt,[a[0]||(a[0]=d("header",{class:"page-header"},[d("div",{class:"container"},[d("h1",null,"教程文档"),d("p",null,"深入学习 XGantt 的各种功能和 API")])],-1)),d("div",at,[d("aside",ot,[d("nav",it,[(i(!0),r(k,null,w(D(p),e=>(i(),r("div",{key:e.id,class:"nav-section"},[d("h3",null,m(e.title),1),d("ul",null,[(i(!0),r(k,null,w(e.sections,o=>(i(),r("li",{key:o.id},[d("a",{href:`#${o.id}`,class:E({active:l.value===o.id}),onClick:R(h=>s(o.id),["prevent"])},m(o.title),11,rt)]))),128))])]))),128))])]),d("main",null,[(i(!0),r(k,null,w(D($),e=>(i(),r("section",{key:e.id,class:"tutorials-main"},[d("h1",st,m(e.title),1),(i(!0),r(k,null,w(e.sections,o=>(i(),r("div",{key:o.id,id:o.id,class:"tutorial-section"},[d("h2",null,m(o.title),1),U(et,{section:o},null,8,["section"])],8,lt))),128))]))),128))])])]))}}),pt=O(ct,[["__scopeId","data-v-e766eae9"]]);export{pt as default};
