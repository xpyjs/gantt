...
export interface IGanttOptions {
  ...
  date: {
     ...
     *
     * 取值含义：
     * @params 'start' - 缺失精度位补为该位的起始值（0）
     * @params 'end' - 缺失精度位补为该位的最大值（时=23，分/秒=59）。
     *                 含尾语义：结束时间保持在尾单位内（如 day 粒度的
     *                 '2026-08-18' 解析为 2026-08-18 23:59:59），时长
     *                 按“从起始秒起数的完整秒数”计算，整单位任务的
     *                 duration 为整数；不足尾单位最后一秒的为小数
     ...
     *
     * @description 默认（endOfAll=false）仅对字符串原始值按其给出精度补全缺失位，已有位保留；
     *              Date/number 不调整。无缺失位时不补全，保持原始解析值。
     * @description 未配置 endOf 时，所有结束时间保持原始解析值。
     * @description 拖拽等交互产生的时间按最小单位网格对齐，更新的数据会被一并抛出供外部处理保存。
     *
     * @example
     * ```ts
     ...
     * ]
     * // 'end' => 任务1 占满 '2024-01-01' 当天，结束时间为 '2024-01-01 23:59:59'，
     * //          duration 为 1、视觉宽度为 1 格。原始数据 '2024-01-01' 不变。
     * // 'end' => 任务2 的结束时间为 '2024-01-02 12:59:59'（13:00:00 为第一秒，
     * //          数满 1 天），duration 为 1。
     * // [0, 12, 30] => 任务1 解析为 '2024-01-01 00:12:30'，duration 不足 1 天
     * ```
     */
    endOf?: 'start' | 'end' | [number, number, number];
    /**
     * 是否强制对所有任务结束时间应用 endOf 解析规则。
     *
     * @default false
     *
     * @description 默认为 false，仅对字符串原始值按其给出精度补全缺失位（Date/number 不调整）。
     * @description 设置为 true 时，忽略精度感知，对所有任务的结束时间（含 Date/number）强制按底层单位全位应用 endOf 调整。
     *
